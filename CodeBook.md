#Project Description
Create a tidy data set from data collected using Samsung Galaxy S II smartphones and downloaded from https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip:.
...
Contents of the README.txt file included with the dataset .zip file downloaded from https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip:
...
From the features_info.txt file included with the dataset .zip file downloaded from https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip:
...
The set of variables that were estimated from these signals are (emphasis added and information collapsed): 
...
2. Use read.table to read the following files included in the .zip file in to R:  X_train.txt and  X_test.txt (observations); y_train.txt and y_test.txt (activity performed during each observation, integer code); subject_train.txt and subject_test.txt (subject performing activity during observation_, activity_labels.txt (description of activity corresponding to integer code) .
...
8. Take a subset of just the observation variables with both a mean and a standard deviation in the combined test and training data frames.  Include subject and activity columns.
9. Load the dplyr package
10. Merge the activity_labels data frame with the data frame created in step 7.  This step is done after all the data is combined and subsetted to avoid reordering the data.
11. Rename the observation columns to reflect the fact that the original data has been averaged.
12. Group the data frame created in step 9 by Subject, Activity Code, and Activity, then summarize the data by the groups and calculate the mean of each observation for each subject/activity/activity code combination.
...
1. The variable names provided by the authors did a good job of describing the variables in a compact manner.   
2. The authors are subject matter experts in the field and I am not; therefore I did not feel like questioning their choices.
...
I felt that Project Requirements  item 2 means that only observations with  both a mean and a standard deviation should be chosen.  Therefore I chose variables from this list where both means and standard deviations had been calculated:
tBodyAcc-XYZ
tGravityAcc-XYZ
tBodyAccJerk-XYZ
tBodyGyro-XYZ
...
I chose to retain both the integer activity code and the activity label because I wanted the option to sort
numerically or alphabetically.  In addition, using a number in a condition takes less space than typing in a text description.

The tidy dataset has 180 observations and 69 variables.  There is one subject/activity combination per 
row and each variable is in its own column.  Neither the README nor the features_info file had details on the units of 
measurement.  The variable names were created by adding “Average(“ at the beginning of the original variable name and closing the parentheses at the end of the original variable name.
...
Sources:  The Elements of Data Analytic Style  (Jeff Leek), Code Book Template (https://gist.github.com/JorisSchut), Getting and Cleaning the Assignment  (https://thoughtfulbloke.wordpress.com/; thoughtfulbloke aka David Hood)
